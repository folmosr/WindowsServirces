    ...
    public class Producer
    {
        ...
        private void OnBeginSendDocuments(object source, ElapsedEventArgs e)
        {
            try
            {
                ...
                SVCDocumentosElectronicos.MensajeWCFOfEnviosPendientesPosteoDIAN respuesta = client.ObtenerDocumentosPendientesEnvio();
                ...
                {
                    configurador.LogWriter.Write(new LogEntry() { Message = String.Format("Documentos pediente de posteo {0}", respuesta.Contenido.Length.ToString()), Categories = new List<string> { "eventLog" }, Priority = 1, ProcessName = "Alimentando Queue" });
                    if (queue.GetAllMessages().Length > 0)
                    {
                        queue.Purge();
                    }

                    foreach (SVCDocumentosElectronicos.EnviosPendientesPosteoDIAN entidad in respuesta.Contenido)
                    {
                        ...
                        using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required))
                        {
                            ...
                            scope.Complete();
                            configurador.LogWriter.Write(new LogEntry() { Message = String.Format("Se mandó a la cola {0}, el documento {1} pendiente posteo", queue.QueueName, entidad.Id), Categories = new List<string> { "eventLog" }, Priority = 1, ProcessName = "Alimentando Queue" });
                        }
